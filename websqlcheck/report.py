...
class Reporter:
    ...
    def write(self, path: str, fmt: str = "json"):
        ...
        if fmt == "csv":
            ...
            return
        if fmt == "html":
            body = "".join(f"<tr><td>{html.escape(str(x['technique']))}</td><td>{html.escape(str(x['parameter']))}</td><td><pre>{html.escape(str(x['payload']))}</pre></td><td>{html.escape(str(x['url']))}</td><td>{html.escape(str(x['confidence']))}</td><td>{x['score']}</td><td><pre>{html.escape(json.dumps(x['evidence'], indent=2))}</pre></td></tr>" for x in self.records())
            page = "<!doctype html><html><head><meta charset='utf-8'><title>WebSQL Check Report</title><style>body{font-family:system-ui;margin:2rem}table{border-collapse:collapse;width:100%}th,td{border:1px solid #ccc;padding:.5rem;text-align:left;vertical-align:top}pre{white-space:pre-wrap}</style></head><body><h1>WebSQL Check Report</h1><table><thead><tr><th>Technique</th><th>Parameter</th><th>Payload</th><th>URL</th><th>Confidence</th><th>Score</th><th>Evidence</th></tr></thead><tbody>" + body + "</tbody></table></body></html>"
